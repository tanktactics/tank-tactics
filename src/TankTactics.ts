...
export class TankTacticsGame extends (EventEmitter as new () => IEventEmitter<IEvents>) implements Game {
    ...
    private intervalFunction() {
        ...
        console.log(`Giving points at ${new Date().toLocaleString('nl')} for ${this.name}`);
    ...
    doStateCheck() {
        if (this.state !== 'ongoing') return false;
    ...
    walkPlayer(id: number, dir: Direction, stepCount: number = 1) {
        ...
        while (stepCount > 0) {
            ...
            if (p.points > 0) {
                ...
                if (newX < 0 || newY < 0 || newX > this.boardWidth - 1 || newY > this.boardHeight - 1)
                    newPosIsValid = false;
                ...
                return `Na ${pointsTaken} stappen naar ${dir} heb je geen punten meer, dat is kut...`;
        }
